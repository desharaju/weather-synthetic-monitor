...
- **Zero Database Required**: Uses in-memory ring buffers backed by a simple JSON file on disk (`data/results.json`).
...
You can deploy this project for free on [Render](https://render.com) to get a live, public URL.
...
4. Configure the service:
   ...
   - **Instance Type**: `Free`
5. (Optional) Click **Advanced** and add Environment Variables (e.g., `MONITOR_CITY`).
6. Click **Create Web Service**.

Render will automatically install the dependencies, start the server, and provide you with an `https://...onrender.com` URL! Every time you push to the `main` branch, Render will auto-deploy your updates.

---

## 🛠️ Architecture 

- `src/monitor.js` — Core execution engine. Uses `fetch` with an abort controller, measures timing, and runs an array of validator functions.
- `src/checks/weather.js` — Defines the specific URLs and validation logic (Current Weather, 7-Day Forecast, Geocoding).
- `src/alerts.js` — Compares check results against thresholds and handles console logging and incident creation.
- `src/store.js` — Handles data lifecycle (keeping only the last 1000 results to prevent massive memory usage) and disk persistence.
